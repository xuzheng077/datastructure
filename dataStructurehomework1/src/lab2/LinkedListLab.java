...
public class LinkedListLab<AnyType> implements Iterable<AnyType> {
    ...
    public AnyType kthToLast(int k) {
        ...
        //fast runner move k steps further
        while (k > 0 && fast != null) {
            fast = fast.next;
            k--;
        }
        //if fast reaches end before k steps
        if (k != 0) {
            return null;
        }
        //fast and slow move together until fast reaches end
        while (fast != null) {
            fast = fast.next;
            slow = slow.next;
        }
        //if slow is null, return null, else return data
